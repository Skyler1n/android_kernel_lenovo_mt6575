...
extern int vfs_statfs64(struct dentry *dentry, struct statfs64 *buf);
...
ssize_t vfs_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if(!memcmp(file->f_path.mnt->mnt_devname,"/dev/block/vold",15)) //sdcard error on write
	{
        bwsdcard=true;
		time1 = sched_clock();
		    printk(KERN_DEBUG " sectors start write %s PID %x , time %lld\n",file->f_path.dentry->d_name.name,tsk->pid,time1);
	}	
#endif	
	...
	if (!memcmp(file->f_path.mnt->mnt_mountpoint->d_name.name, "data", 5)) {
		store -= count;	
		if (store  <= CHECK_1TH) {		
			vfs_statfs64(file->f_path.dentry, &stat);
...
#ifdef ANRTEST
	if(bwsdcard) //sdcard error on write
	{
		time2 = sched_clock();
		bwsdcard = false;
        if((time2 - time1) > (unsigned long long )20000000)
      	printk(KERN_DEBUG " sectors write %s PID %x ,ret %x time %lld\n",file->f_path.dentry->d_name.name,tsk->pid,ret,(time2-time1));
	}	
#endif	
	return ret;
}
