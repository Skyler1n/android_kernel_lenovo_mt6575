...
int yaffs_CheckpointSave(yaffs_Device *dev)
{
	...
	T(YAFFS_TRACE_ALWAYS, (TSTR("save exit:%s isCheckpointed %d"TENDSTR), dev->param.name,dev->isCheckpointed));
...
{
	register unsigned int aseq = ((yaffs_BlockIndex *)a)->seq;
	register unsigned int bseq = ((yaffs_BlockIndex *)b)->seq;
...
static int yaffs_ScanCheckEraseClean(struct yaffs_DeviceStruct *dev,
...
int yaffs2_ScanBackwards(yaffs_Device *dev)
{
	...
	for (blk = dev->internalStartBlock; blk <= dev->internalEndBlock; blk++) {
		...
		for(c=0;c<dev->param.nChunksPerBlock;c++){
				yaffs_ScanCheckEraseClean(dev,blk*dev->param.nChunksPerBlock+c,c);
				}
#endif	
			...
			if (! yaffs_EraseBlockInNAND(dev, blk)) {
				...
				T(YAFFS_TRACE_ERROR | YAFFS_TRACE_BAD_BLOCKS,
			  		(TSTR("**>> Erasure failed %d" TENDSTR), blk));
				continue;
				}
			T(YAFFS_TRACE_SCAN_DEBUG,
			...
			if (!tags.chunkUsed) {
				...
				if(check_on&&(check_block>0)){					
					...
					for(i=0;i<8;i++)
					{
						...
						result = yaffs_ReadChunkWithTagsFromNAND(dev,chunk,buffer,&tempTags);
						if(YAFFS_ECC_RESULT_UNFIXED == tempTags.eccResult)
						{
							retv = -1;
							break;
						}
						else if(memcmp(buffer,chunkData,dev->nDataBytesPerChunk) ||
					...
					if(retv<0){
						...
						printk(KERN_ERR"yaffsdebug re-check page fail!retv:%d i:%d eccr:%d chunk:%d addr:0x%x cb:%d sn:%d\n",\
							retv,i,tags.eccResult,chunk,chunk*dev->nDataBytesPerChunk,check_block,bi->sequenceNumber);
						...
						//if(-2 == retv)
						{
							printk(KERN_ERR"dump data cant fixed ecc!chunk:%d addr:0x%x\n",chunk,chunk*dev->nDataBytesPerChunk);
							mtk_dump_byte(chunkData,dev->nDataBytesPerChunk,0);		
							printk(KERN_ERR"dump tag\n");
							mtk_dump_byte(&tags , sizeof(yaffs_ExtendedTags),0);
							
							printk(KERN_ERR"dump tmpdata cant fixed ecc!chunk:%d addr:0x%x\n",chunk,chunk*dev->nDataBytesPerChunk);
							mtk_dump_byte(buffer,dev->nDataBytesPerChunk,0);		
							printk(KERN_ERR"dump tmptag\n");
							mtk_dump_byte(&tempTags,sizeof(yaffs_ExtendedTags),0);
						}
#endif					
