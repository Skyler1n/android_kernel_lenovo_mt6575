...
static unsigned int yaffs_mvg_test_debug_mode = 0;
/* Module Parameters */
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 5, 0))
...
static void yaffs_FlushSuperBlock(struct super_block *sb, int do_checkpoint)
{
	...
	T(YAFFS_TRACE_ALWAYS,
				  (TSTR("yaffs: %s yaffs_FlushSuperBlock %x" TENDSTR),dev->param.name,do_checkpoint));
				...
				int request_checkpoint)
{
	...
	if (sb->s_dirt || do_checkpoint) {
		T(YAFFS_TRACE_ALWAYS,
				  (TSTR("yaffs: %s sync do_checkpoint %x,gc_urgen %x" TENDSTR),dev->param.name,do_checkpoint,gc_urgent));
						...
						void *data, int silent)
{
	...
	T(YAFFS_TRACE_ALWAYS,
		(TSTR("yaffs: Attempting MTD mount of %u.%u,\"%s\" debug_mode:0x%x\n"),
