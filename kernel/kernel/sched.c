...
extern int mtsched_mon_enabled;
...
#endif
#ifdef CONFIG_PREEMPT_MONITOR
		if(unlikely(1 == mtsched_mon_enabled || 3 == mtsched_mon_enabled))
...
void __kprobes sub_preempt_count(int val)
{
	...
	if (preempt_count() == (val & ~PREEMPT_ACTIVE)){
		...
		if(unlikely(t > 30000000 && current->t_add_prmpt!=0 ))
		{
			...
			printk("[%d:%s]Preempt Duration: %llu ns > 55 ms \n",current->pid, current->comm, t);
		    }else{
			printk("[%d:%s]Preempt Duration: %llu ns > 30 ms \n",current->pid, current->comm, t);
		    }
		    dump_stack();
		}
	    }    
