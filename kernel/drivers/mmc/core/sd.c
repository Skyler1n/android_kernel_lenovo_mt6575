...
#define SD_TUNING_CLOCK_AT_INITIATION 1
	...
	struct mmc_card *oldcard)
{
 ...
 if(!switch_hs_flag){
	...
	if ((host->caps & MMC_CAP_4_BIT_DATA) &&
		...
		/*workaround for cmd_sel: host latches response wihtout 1T delay, and retry ACMD6*/
		if(err) 
		{		
			printk("ACMD6 failed, change cmd_sel from %d to %d\r\n", host->cmd_sel, !host->cmd_sel);	
			mmc_set_cmd_sel(host, !host->cmd_sel);
			err = mmc_app_set_bus_width(card, MMC_BUS_WIDTH_4);
		}
...
static int mmc_sd_resume(struct mmc_host *host)
{
	...
	while (retries) {
		...
		if (err) {
			...
			mdelay(5);
			retries--;
#ifdef SD_TUNING_CLOCK_AT_INITIATION
			if(retry == 3){
				printk("[%s]: failed to init SD card with high speed, retry with normal speed\r\n", __func__);
				host->f_max = 25000000;
			}
#endif
			continue;
