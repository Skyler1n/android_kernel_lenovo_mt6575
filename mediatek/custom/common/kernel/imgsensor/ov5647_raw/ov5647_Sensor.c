...
#ifdef OV5647_DRIVER_TRACE
	SENSORDB("OV5647_Write_Shutter:%x \n",iShutter);
#endif
...
#if 0
    OV5647_write_cmos_sensor(0x3501, (iShutter >> 8) & 0xFF);	
    OV5647_write_cmos_sensor(0x3502, (iShutter ) & 0xFF);
#else
...
#ifdef OV5647_DRIVER_TRACE
	SENSORDB("OV5647_sensor.frame_height:%d \n",OV5647_sensor.frame_height);
...
static void OV5647_Set_Dummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
...
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647_Set_Dummy:iPixels:%x; iLines:%x \n",iPixels,iLines);
#endif
	...
	if((line_length == OV5647_sensor.line_length)&&(frame_height == OV5647_sensor.frame_height))
		return ;

	OV5647_sensor.line_length = line_length;
	OV5647_sensor.frame_height = frame_height;
...
{
#ifdef OV5647_DRIVER_TRACE
	SENSORDB("set_OV5647_shutter:%x \n",iShutter);
#endif
	if (OV5647_sensor.shutter == iShutter)
		return ;
	//if (iShutter > 0x2300)
		//iShutter = 0x2292;
...
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647Reg2Gain:Reg:%x; Gain:%x \n",iReg,iGain);
#endif
    return iGain ;
}
 ...
 kal_uint8 OV5647Gain2Reg(const kal_uint16 iGain)
{
	...
	iReg = iReg & 0xFF;
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647Gain2Reg:iGain:%x; iReg:%x \n",iGain,iReg);
#endif
...

{
   kal_uint8 iReg;
#ifdef OV5647_DRIVER_TRACE
   SENSORDB("OV5647_SetGain:%x;\n",iGain);
#endif
   // (OV5647_sensor.gain == iGain)
	// return ;
...
static void OV5647_set_isp_driving_current(kal_uint8 current)
{
#ifdef OV5647_DRIVER_TRACE
   SENSORDB("OV5647_set_isp_driving_current:current:%x;\n",current);
#endif
...
#ifdef OV5647_DRIVER_TRACE
   SENSORDB("OV5647_night_mode:enable:%x;\n",enable);
#endif
...
static void OV5647_Sensor_Init(void)
{
	...
	OV5647_write_cmos_sensor(0x3013, 0x04);
...
static void OV5647_Sensor_1M(void)
{
	...
	OV5647_write_cmos_sensor(0x380c, 0x06); /*	TIMING HTS 0x065e */
	OV5647_write_cmos_sensor(0x380d, 0x5e); /*	TIMING HTS 1630 */
	...
	OV5647_write_cmos_sensor(0x3809, 0x00); /* TIMING X OUTPUT SIZE  1280 */
	OV5647_write_cmos_sensor(0x380a, 0x03); /* TIMING Y OUTPUT SIZE  0x03c0 */
	OV5647_write_cmos_sensor(0x380b, 0xc0); /* TIMING Y OUTPUT SIZE  960 */
	OV5647_write_cmos_sensor(0x3800, 0x00); /* TIMING X ADDR START	*/
	OV5647_write_cmos_sensor(0x3801, 0x18); /* TIMING X ADDR START	*/
	OV5647_write_cmos_sensor(0x3802, 0x00); /* TIMING Y ADDR START	*/
	OV5647_write_cmos_sensor(0x3803, 0x0e); /* TIMING Y ADDR START	*/
	OV5647_write_cmos_sensor(0x3804, 0x0a); /* TIMING X ADDR END  */
	OV5647_write_cmos_sensor(0x3805, 0x27); /* TIMING X ADDR END  */
	OV5647_write_cmos_sensor(0x3806, 0x07); /* TIMING Y ADDR END  */
	OV5647_write_cmos_sensor(0x3807, 0x95); /* TIMING Y ADDR END  */
...
UINT32 OV5647Capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	const kal_uint16 pv_line_length = OV5647_sensor.line_length;
	//const kal_uint32 pv_pclk = OV5647_sensor.pclk;
	kal_uint16 shutter = OV5647_sensor.shutter;
	kal_uint16 dummy_pixel, cap_fps;
#ifdef OV5647_DRIVER_TRACE
	SENSORDB("OV5647Capture \n");
#endif
...
{
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647GetInfo£¬FeatureId:%d\n",ScenarioId);
#endif
#if 0
...
{
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647Control£¬FeatureId:%d\n",ScenarioId);
#endif		
	switch (ScenarioId)
	{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
...
#ifdef OV5647_DRIVER_TRACE
	SENSORDB("OV5647SetVideoMode£¬u2FrameRate:%d\n",u2FrameRate);
#endif		
...
#ifdef OV5647_DRIVER_TRACE
	//SENSORDB("OV5647FeatureControl£¬FeatureId:%d\n",FeatureId);
#endif		
	switch (FeatureId)
	{
