...
sub write_tag_one_element()
{
    ...
    print FILE pack ("L", hex ($EMI_CONA_VAL[$id])) ;         # EMI_CONA_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_DRVCTL0_VAL[$id])) ;    # DRAMC_DRVCTL0_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_DRVCTL1_VAL[$id])) ;    # DRAMC_DRVCTL1_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_DLE_VAL[$id])) ;        # DRAMC_DLE_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_ACTIM_VAL[$id])) ;      # DRAMC_ACTIM_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_GDDR3CTL1_VAL[$id])) ;  # DRAMC_GDDR3CTL1_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_CONF1_VAL[$id])) ;      # DRAMC_CONF1_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex ($DRAMC_DDR2CTL_VAL[$id])) ;    # DRAMC_DDR2CTL_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex ($DRAMC_TEST2_3_VAL[$id])) ;    # DRAMC_TEST2_3_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_CONF2_VAL[$id])) ;      # DRAMC_CONF2_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_PD_CTRL_VAL[$id])) ;    # DRAMC_PD_CTRL_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ($DRAMC_PADCTL3_VAL[$id])) ;    # DRAMC_PADCTL3_VAL
    
    $fs = $fs + 4 ;
    print FILE pack ("L", hex ($DRAMC_DQODLY_VAL[$id])) ;     # DRAMC_DQODLY_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex ($DRAMC_ADDR_OUTPUT_DLY[$id])) ;        # DRAMC_ADDR_OUTPUT_DLY
    $fs = $fs + 4 ;

    print FILE pack ("L", hex ($DRAMC_CLK_OUTPUT_DLY[$id])) ;        # DRAMC_CLK_OUTPUT_DLY
    $fs = $fs + 4 ;
    ...
    {
        #ddr2
        print FILE pack ("L", hex ($DDR2_MODE_REG1[$id])) ;        # DDR2_MODE_REG1
        $fs = $fs + 4 ;

        print FILE pack ("L", hex ($DDR2_MODE_REG2[$id])) ;        # DDR2_MODE_REG2
        $fs = $fs + 4 ;

        print FILE pack ("L", hex ($DDR2_MODE_REG3[$id])) ;        # DDR2_MODE_REG3
        $fs = $fs + 4 ;

        print FILE pack ("L", hex ($DDR2_MODE_REG10[$id])) ;        # DDR2_MODE_REG10
        $fs = $fs + 4 ;

        print FILE pack ("L", hex ($DDR2_MODE_REG63[$id])) ;        # DDR2_MODE_REG63
        $fs = $fs + 4 ;
    }
    elsif ($DEV_TYPE2[$id] == "01")
    {   # ddr1
        print FILE pack ("L", hex ($DDR1_MODE_REG[$id])) ;        # DDR1_MODE_REG;
        $fs = $fs + 4 ;

        print FILE pack ("L", hex ($DDR1_EXT_MODE_REG[$id])) ;        # DDR1_EXT_MODE_REG
        $fs = $fs + 4 ;
