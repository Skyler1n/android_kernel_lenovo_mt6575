...
ANDROID_NATIVE_TARGETS := \
         update-api \
         cts sdk win_sdk otapackage banyan_addon \
         snod bootimage systemimage recoveryimage secroimage \
         factoryimage userdataimage
...
check-modem: PRIVATE_MODEM := \
    ...
    ifneq (,$(findstring mt6573,$(CUSTOM_MODEM)))
check-modem: PRIVATE_DSP_ROM := \
             $(if $(wildcard $(MTK_ROOT_CUSTOM)/$(PROJECT)/modem/$(strip $(CUSTOM_MODEM))/DSP_ROM_E2), \
               $(MTK_ROOT_CUSTOM)/$(PROJECT)/modem/$(strip $(CUSTOM_MODEM))/DSP_ROM_E2, \
               $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))/DSP_ROM_E2 \
              )
check-modem: PRIVATE_MODEM := \
             $(if $(wildcard $(MTK_ROOT_CUSTOM)/$(PROJECT)/modem/$(strip $(CUSTOM_MODEM))/modem_E2.img), \
               $(MTK_ROOT_CUSTOM)/$(PROJECT)/modem/$(strip $(CUSTOM_MODEM))/modem_E2.img, \
               $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))/modem_E2.img \
              )
    endif
check-modem:
	$(hide) echo $(SHOWTIME) $@ing ...
