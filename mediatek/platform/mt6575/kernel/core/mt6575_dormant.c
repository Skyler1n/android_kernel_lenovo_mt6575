...
static void save_pl310(u32 *pointer, unsigned pl310_address)
{
    ...
    copy_words(context->lockdown, pl310->lockdown, 16);
...
static void restore_pl310(u32 *pointer, unsigned pl310_address, int dormant)
{
    ...
    copy_words(pl310->lockdown, context->lockdown, 16);
...
#endif
        restore_gic_distributor_shared(cpu_cluster.context.gic_dist_shared_data, cpu_cluster.ic_address);
        restore_a9_global_timer(cpu_cluster.context.global_timer_data, cpu_cluster.scu_address);
#if MAX_CPUS != 1
    }
#endif

    restore_gic_distributor_private(cpu.context.gic_dist_private_data, cpu_cluster.ic_address);
    restore_gic_interface(cpu.context.gic_interface_data, cpu.ic_address);

    restore_a9_other(cpu.context.other_data);

    restore_vfp(cpu.context.vfp_data);

    restore_a9_timers(cpu.context.timer_data, cpu_cluster.scu_address);

    restore_performance_monitors(cpu.context.pmu_data);

    dormant_ret_flag = 1;

    restore_banked_registers(cpu.context.banked_registers);
}
...
int cpu_power_down(int mode)
{
    ...
    platform_save_context(&cpu_cluster, cpu_cluster.cpu_table);
