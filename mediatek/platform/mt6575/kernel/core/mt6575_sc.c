...
static u32 sc_get_wake_period(bool first)
{
	...
	sec = 300;
#else
	sec = get_dynamic_period(first, 300, 1);	/* battery decreases 1% */
#endif
	if (unlikely(sec == 0 || sec > 36 * 3600))	/* max period is 36.4 hours */
		sc_xfatal("wake period %u is out of range\n", sec);
	else
		sc_xinfo("sec = %u\n", sec);
...
wake_reason_t sc_go_to_sleep(unsigned int pwrlevel)
{
	...
	mt65xx_irq_unmask(MT6575_SLEEP_IRQ_ID);
...
MODULE_DESCRIPTION("MT6575 SLPCTRL Driver v3.5");
