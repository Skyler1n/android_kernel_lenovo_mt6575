...
void enable_low_power_settings(void)
{
    ...
    mt6575_irq_unmask(MT6575_KP_IRQ_ID);
}
#endif
...
void mt6575_dcm_init(void)
{
    ...
    sysfs_create_file(power_kobj, &dcm_state_attr.attr);
    sysfs_create_file(power_kobj, &idle_state_attr.attr);
    sysfs_create_file(power_kobj, &dpidle_state_attr.attr);
}
