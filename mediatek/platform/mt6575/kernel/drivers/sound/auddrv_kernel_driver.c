...
void AudDrv_Init_DL1_Stream(kal_uint32 Afe_Buf_Length)
{
    ...
    */
    {//Audio DL1 master mode used
            ...
            if (DL1InPhyBufAddr == NULL)
            {
                kal_uint32 u4PhyAddr = 0;
                xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Init_DL1_Stream MTxxx_SYSRAM_ALLOC \n");
                u4PhyAddr = (void*)SYSRAM_AUDIO_ALLOC_TIMEOUT(AFE_dl_Control->rBlock.u4BufferSize,32,AUD_SYSRAM_ALLOC_TIMEOUT);
            ...
            AFE_dl_Control->rBlock.pucVirtBufAddr = (AFE_dl_Control->rBlock.pucPhysBufAddr - MM_SYSRAM_BASE_PA) + SYSRAM_BASE;
#else
    ////////////
...
*/
   ...
   if(Afe_Block->u4DataRemained <= Afe_consumed_bytes)
   {
      ...
      xlog_printk(ANDROID_LOG_ERROR, "Sound","+Tasklet underflow ReadIdx:%x WriteIdx:%x, DataRemained:%x, HW_cons:%x \n",
      ...
      xlog_printk(ANDROID_LOG_ERROR, "Sound","-Tasklet 2underflow ReadIdx:%x, WriteIdx:%x, DataRemained:%x \n",
...
// for DL1 output SRC
      ...
      case START_DL1_STREAM:
      {
        ...
        if(!Flag_Aud_DL1_SlaveOn)
        {
            if (AFE_dl_Control->rBlock.pucPhysBufAddr==NULL){
...
//   HW_Cur_ReadIdx = Afe_Get_Reg(AFE_DL1_CUR);  // for debugging
   ...
   while(count)
   {
      ...
      if(count != 0)
      {
         ...
         if(ret <= 0)
         {
             xlog_printk(ANDROID_LOG_ERROR, "Sound","AudDrv_write wait_event_interruptible_timeout, No Audio Interrupt!\n");
//           AudDrv_AudReg_Log_Print();
             if( (SPH_Ctrl_State.bSpeechFlag==false) && (SPH_Ctrl_State.bBgsFlag==false) && (SPH_Ctrl_State.bTtyFlag==false)
                 ...
                 AFE_dl_Control->rBlock.u4fsyncflag    = false;
                 spin_unlock_bh(&auddrv_lock);
                 xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write wait timeout,restart \n");
                 Afe_Set_Reg(AFE_DAC_CON0,0x2,0x2);
                 Afe_Set_Reg(AFE_IRQ_CON,0x1,0x1);
             }else{
                 xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write wait timeout, not restart, bBgsFlag:%d,bRecordFlag:%d,bSpeechFlag:%d,bTtyFlag:%d,bVT:%d,bAudio:%d \n",
                        SPH_Ctrl_State.bBgsFlag,
                        SPH_Ctrl_State.bRecordFlag,
                        SPH_Ctrl_State.bSpeechFlag,
                        SPH_Ctrl_State.bTtyFlag,
