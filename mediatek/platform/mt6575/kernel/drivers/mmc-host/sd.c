...
#define msdc_clk_stable(mode, div) \
    do { \
        int retry = 3, cnt = 1000; \
        sdr_set_field(MSDC_CFG, MSDC_CFG_CKDIV,(div + 1) % 0xff); \
		...
		if(host->id == 0 && 
			...
			parition_access == 0
				...
				offset =  (MBR_START_ADDRESS_BYTE - (ext_csd[EXT_CSD_BOOT_SIZE_MULT]* 128 * 1024
                                ...
                                +ext_csd[EXT_CSD_GP4_SIZE_MULT + 0]))/512;
				printk("[SD%d]Address offset in USER REGION%x\n",host->id,offset);
				if((int)offset < 0){
					printk("[SD%d] XXX Address offset error(%d),please check MBR start address!!\n",host->id,(int)offset);
					goto done;
					}
				cmd->arg = offset;
			}
...
stop:        
    ...
    if (data != NULL) {
        ...
        //INIT_MSG("CMD<%3d> arg<0x%8x> data<%s %s> blksz<%d> block<%d>",cmd->opcode, cmd->arg, (dma? "dma":"pio"), 
        //        (read ? "read ":"write") ,data->blksz, data->blocks);  
    } else {
        //INIT_MSG("CMD<%3d> arg<0x%8x>",cmd->opcode, cmd->arg);        	
    }
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
    ...
    if (host->hw->flags & MSDC_CD_PIN_EN) {
        spin_lock_irqsave(&host->lock, flags);
        if(host->hw->flags & MSDC_SPE){
            present = 1;
            printk("msdc%d detect card\r\n",host->id);
        } else {
...
static inline int emmc_proc_info(char *buf, struct hd_struct *this)
{
	...
	return sprintf(buf, "emmc_p%d: %8.8x %8.8x \"%s\"\n", this->partno,
		       ...
		       (unsigned int)this->nr_sects, this->partition_name);
}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
    ...
    mt65xx_irq_set_sens((unsigned int)irq, MT65xx_LEVEL_SENSITIVE);
    mt65xx_irq_set_polarity((unsigned int)irq, MT65xx_POLARITY_LOW);
