...
{ 
    ...
    {
    //************analog switch*******************/
    //**MT6575 EVB**: 1, MIC channel; 0, TV channel (default)
    //**MT6575 Phone**: 1, TV channel; 0, MIC channel(default)
    #ifndef MT6575_EVB_MIC_TV_CHA
    hwSPKClassABAnalogSwitchSelect(ACCDET_MIC_CHA);
    #else
    hwSPKClassABAnalogSwitchSelect(0x1);
    ...
    switch(cmd)
    {
        case ACCDET_INIT :
			...
			{	
				...
				ret =  request_irq(MT6575_ACCDET_IRQ_ID, accdet_irq_handler, 0, "ACCDET", NULL);
				if(ret)
				{
...
static int accdet_resume(struct platform_device *dev) // wake up
{
    ...
    ACCDET_DEBUG("[Accdet]accdet_resume: ACCDET_STATE_SWCTRL=[0x%x]\n", 
        INREG32(ACCDET_STATE_SWCTRL));
	ACCDET_DEBUG("[Accdet]resum:sample_in:%x!\n curr_in:%x!\n mem_in:%x!\n FSM:%x!\n"
		, INREG32(ACCDET_SAMPLE_IN)
		,INREG32(ACCDET_CURR_IN)
		,INREG32(ACCDET_MEMORIZED_IN)
		,INREG32(ACCDET_BASE + 0x0050));
...
MODULE_DESCRIPTION("MTK MT6575 ACCDET driver");
