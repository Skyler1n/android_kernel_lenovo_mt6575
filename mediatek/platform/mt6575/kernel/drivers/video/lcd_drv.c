...
#define DBI_LOG(fmt, arg...) \
    do { \
        if (dbi_log_on) printk("[dbi log]"fmt, ##arg); \
    }while (0)
...
LCD_STATUS LCD_PowerOn(void)
{
	...
	DBI_LOG("lcd will be power on\n");
		...
		if(ret > 0)
		{
			printk("[LCD]power manager API return FALSE\n");
		}
#endif        
...
void LCD_ProtectTranslationFault(void)
{
	...
	for(i=0;i<FB_LAYER;i++){
		if(LCD_IsLayerEnable((LCD_LAYER_ID)i) && !LCD_REG->LAYER[(LCD_LAYER_ID)i].CONTROL.THREE_D){
...
LCD_STATUS LCD_Capture_Videobuffer(unsigned int pvbuf, unsigned int bpp, unsigned int video_rotation)
{
	...
	switch(video_rotation)
	{
		...
		case 3:
			...
			break;
		default:
		    printk("[LCD_Capture_videobuffer] video_rotation = %d error\n", video_rotation);
	}
...
#if defined(MTK_M4U_SUPPORT)
	_m4u_lcdc_func.m4u_dma_cache_maint(M4U_CLNTMOD_LCDC, pvbuf, w*h*bpp/8, DMA_BIDIRECTIONAL);
#endif
...
LCD_STATUS LCD_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	if (!_m4u_lcdc_func.isInit)
	{
		printk("[LCDC init M4U] M4U not been init for LCDC, this should only happened in recovery mode\n");
        unsigned int t;
		unsigned int i,j;
		...
		unsigned int fbv = (unsigned char*)ioremap_cached((unsigned int)LCD_REG->LAYER[FB_LAYER].ADDRESS, fbsize);
	...
	LCD_SetOutputAlpha(0xff);
#if defined(MTK_M4U_SUPPORT)
	_m4u_lcdc_func.m4u_dma_cache_maint(M4U_CLNTMOD_LCDC, pvbuf, DISP_GetScreenHeight()*DISP_GetScreenWidth()*bpp/8, DMA_BIDIRECTIONAL);
#endif
	if(dbi_log_on)
		LCD_DumpRegisters();
...
int m4u_alloc_mva_stub(M4U_MODULE_ID_ENUM eModuleID, const unsigned int BufAddr, const unsigned int BufSize, unsigned int *pRetMVABuf)
{
	if (!_m4u_lcdc_func.isInit)
	{
		printk("%s, Error, M4U has not init func\n", __func__);
		return LCD_STATUS_ERROR;
	}
...
LCD_STATUS LCD_M4U_On(bool enable)
{
	...
	if (!_m4u_lcdc_func.isInit)
	{
		printk("[LCDC M4U ON] M4U not been init for LCDC\n");
		return LCD_STATUS_ERROR;
	}
