...
static u16 aee_pressed_keys;
static bool aee_timer_started;

static inline void kpd_update_aee_state() {
	if(aee_pressed_keys == 0x03) {
		...
		if(aee_timer_started) {
			while(hrtimer_cancel(&aee_timer))
			{
...
{
	printk(KPD_SAY "Power Key generate, pressed=%d\n", pressed);
	if(!kpd_input_dev) {
...
{
	printk(KPD_SAY "PMIC reset Key generate, pressed=%d\n", pressed);
	if(!kpd_input_dev) {
		printk("KPD input device not ready\n");
		return;
	}
