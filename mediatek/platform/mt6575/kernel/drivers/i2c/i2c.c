...
static struct mt6575_i2c_hrtimer_t{
	ktime_t expire;
	u32 data_size;
...
static enum hrtimer_restart mt6575_i2c_hrtimer_func(struct hrtimer *timer)
{
	...
	fifo_left = I2C_FIFO_SIZE - (fifo_stat >> 4) & 0xf;
...
int mt6575_i2c_polling_read(int port, unsigned char addr, unsigned char *buffer, int len)
{
  ...
  unsigned short status; 
	...
	if (in_atomic() || irqs_disabled()) {
	        ret = mutex_trylock(&i2c2_adapter->bus_lock);
	        if (!ret)
		          /* I2C activity is ongoing. */
		        return -EAGAIN;
		} else {
		    mutex_lock_nested(&i2c2_adapter->bus_lock, i2c2_adapter->level);
		}
...
#endif  
	mutex_unlock(&i2c2_adapter->bus_lock);
	return ret;
}
